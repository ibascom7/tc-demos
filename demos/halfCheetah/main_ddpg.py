...
if __name__ == "__main__":
	...
	for seed in range(num_seeds):
		print(f"\n{'='*50}")
		print(f"DDPG - Running seed {seed}")
		print(f"{'='*50}\n")
		...
		if seed == 0:
			record_video(policy, env_name, seed, VIDEOS_DIR, f"ddpg_seed{seed}_0_start")
		...
		for t in range(max_timesteps):
			...
			if (t + 1) % eval_freq == 0:
				...
				np.save(os.path.join(RESULTS_DIR, f"DDPG_HalfCheetah-v5_{seed}"), evaluations)
			...
			if seed == 0:
				for vt in video_timesteps[1:]:
					if t + 1 == vt and vt not in videos_recorded:
						...
						record_video(policy, env_name, seed, VIDEOS_DIR, f"ddpg_seed{seed}_{pct}pct")
		...
		if final_reward > best_reward:
			...
			best_seed = seed
			# Save best model
			policy.save(os.path.join(RESULTS_DIR, "ddpg_best_model"))

	# Record video of the best run
	print(f"\n{'='*50}")
	print(f"Best seed: {best_seed} with reward: {best_reward:.3f}")
	print(f"Recording best DDPG policy video...")
	print(f"{'='*50}\n")
	...
	best_policy = DDPG.DDPG(
		state_dim=state_dim,
		action_dim=action_dim,
		max_action=max_action,
	)
	best_policy.load(os.path.join(RESULTS_DIR, "ddpg_best_model"))
	record_video(best_policy, env_name, best_seed, VIDEOS_DIR, "ddpg_best_final")
